...
jQuery("input.dark_switch").bind("click", function() {
	var circled_switch = jQuery(this).attr('value');

	if(circleed_switch == 'light') {
...
/**
	LAYOUT STYLE [wide|circled]
**/
jQuery("input.circled_switch").bind("click", function() {
	var circled_switch = jQuery(this).attr('value');

	if(circled_switch == 'circled') {
		jQuery("body").removeClass('circled');
		jQuery("body").addClass('circled');
		createCookie("is_circled", 'true', 365);
	} else {
		jQuery("body").removeClass('circled');
		createCookie("is_circled", '', -1);
